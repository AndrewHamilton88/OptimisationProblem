...
namespace ConsoleApplication1
{
    class AllStages
    {
        ...
        public List<List<string>> CheckAllStages()
        {
            ...
            System.IO.StreamReader File = new System.IO.StreamReader(@"TestFile3.txt");

            StreamWriter sw;
            sw = new StreamWriter(@"TestFile4.txt");

            while ((Line = File.ReadLine()) != null)
            {
                string NewStage = "";
                NewStage += Line;
                //List<string> TempStage;
                ...
                {
                    int TempCounter = 0;
                    if (!NewStage.Contains(Convert.ToString(i + 1)))
                    {
                        foreach (string phase in NewStage.Split(','))
                        {
                            int tempno = 0;
                            if (phase == "")
                            {
                            ...
                            tempno = Convert.ToInt16(phase) - 1;

                            if (Combinations[i, tempno])
                            {
                                TempCounter++;
                            }
                            else
                    ...
                    PhaseCounter = NewStage.Split(',').Count() - 1;  //Counts the number of phases in each stage
                    if (TempCounter == PhaseCounter)
                    {
                        NewStage += "," + Convert.ToString(i + 1);
                    }
                }
                Console.WriteLine(NewStage);

                foreach (string phase in NewStage.Split(','))
                {
                    if (phase != "")
                    {
                        SortingList.Add(Convert.ToInt16(phase));
                    }
                }
                SortingList.Sort();
                foreach (int item in SortingList)
                {
                    sw.Write("," + Convert.ToString(item));
                }
                sw.WriteLine();
        ...
        public List<List<int>> StagesGenerator()
        {
            List<List<int>> Results = new List<List<int>>();
            string Line;

            // Read the file and display it line by line.
            System.IO.StreamReader File = new System.IO.StreamReader(@"OptimalCrossroadStages.txt");
            ...
            while ((Line = File.ReadLine()) != null)
            {
                ...
                foreach (string phase in Line.Split(','))
                {
                    if (phase != "")
                    {
                        TempPhaseList.Add(Convert.ToInt16(phase));
                    }
                }
        ...
        /// <summary>
        /// This code simply returns the highest delay value stage
        /// </summary>
        /// <param name="Stages"></param>
        /// <returns></returns>
        public int SimpleHighestStage(List<List<int>> Stages)
        {
            ...
            foreach (List<int> Stage in Stages)
            {
                ...
                Console.WriteLine("Stage " + Counter + ": " + TempStageValue);
            ...
            Console.WriteLine("Best Stage is = " + Convert.ToString(StageNo));
        ...
        public void StageOptions()
        {
            ...
            {
                List<int> TempPhaseList = new List<int>();
                if (Line.Contains("2") && Line.Contains("3") || Line.Contains("5") && Line.Contains("6") || Line.Contains("8") && Line.Contains("9") || Line.Contains("11") && Line.Contains("12"))
                {
                    Results.Add(Line);                    
                }
            }
            ...
            StreamWriter sw;
            sw = new StreamWriter(@"StageOptionTest1.txt");

            int Counter = 0;
            foreach (string Stages in Results)
            {
                Counter++;
                sw.WriteLine(Stages);
            }
